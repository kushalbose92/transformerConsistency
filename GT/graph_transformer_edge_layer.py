...
class GraphTransformerLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, num_heads, manifold, c, dropout=0.0, layer_norm=False, batch_norm=True, residual=True, use_bias=False):
        ...
        if self.c == 0:
            ...
            self.FFN_h_layer1 = HyperbolicLinear(out_dim, out_dim*2, self.manifold, c)
            self.FFN_h_layer2 = HyperbolicLinear(out_dim*2, out_dim, self.manifold, c)
        ...
        if self.c == 0:
            ...
            self.FFN_e_layer1 = HyperbolicLinear(out_dim, out_dim*2, self.manifold, c)
            self.FFN_e_layer2 = HyperbolicLinear(out_dim*2, out_dim, self.manifold, c)
    ...
    def forward(self, g, h, e, node_pos):
        h_in1 = h # for first residual connection
        e_in1 = e # for first residual connection
        if self.c != 0:
            h_in1 = self.manifold.expmap0(h_in1)
            e_in1 = self.manifold.expmap0(e_in1)
        
        # multi-head attention out
        h_attn_out, e_attn_out = self.attention(g, h, e, node_pos)
        ...
        if self.residual:
            if self.c != 0:
                ...
                e = self.manifold.mobius_add(e_in1, e)
                h = self.manifold.logmap0(h)
                e = self.manifold.logmap0(e)
            else:
        ...
        if self.c != 0:
            ...
            e_in2 = self.manifold.expmap0(e_in2)
        ...
        if self.residual:
            if self.c != 0:
                ...
                e = self.manifold.mobius_add(e_in2, e)
                h = self.manifold.logmap0(h)
                e = self.manifold.logmap0(e)
            else:
